...
│   ├── publish-crate.yml     # tag-triggered crates.io publish
│   └── release.yml           # tag-triggered binary release (autogenerated by cargo-dist)
├── release/
...
Two workflows fire on every `v*.*.*` tag and run independently:

1. `.github/workflows/publish-crate.yml` — crates.io path:
   - Verifies the tag matches the `Cargo.toml` version.
   - Runs the full check suite (`fmt`, `clippy`, `test`, `doc`, `deny`).
   - `cargo publish`es to crates.io using the `CARGO_REGISTRY_TOKEN`
     secret.
2. `.github/workflows/release.yml` — binary path, **autogenerated by
   cargo-dist**. Builds per-target archives for Linux (x86_64 /
   aarch64, gnu + musl), macOS (x86_64 / aarch64), and Windows
   (x86_64); uploads them plus a shell installer, a PowerShell
   installer, and a Homebrew formula to a GitHub Release; and writes
   release notes from the `## [X.Y.Z]` CHANGELOG section.
...
The binary release pipeline is configured in `dist-workspace.toml` at
the repo root and the `[profile.dist]` section of `Cargo.toml`. Never
hand-edit `.github/workflows/release.yml` — it's regenerated. To
change targets, installers, or the cargo-dist version:

1. Edit `dist-workspace.toml`.
2. Run `dist generate` (from cargo-dist ≥ 0.28, the binary is `dist`).
3. Commit the regenerated workflow alongside the config change.

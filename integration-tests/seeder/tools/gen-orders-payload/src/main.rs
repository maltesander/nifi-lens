...
// 10000 records ≈ 822 KiB CSV → ~2 MiB JSON / Parquet / Avro outputs.
// Sized to comfortably fit inside the 8 MiB MODAL_CHUNK_BYTES limit so
// the diff modal can decode and render both sides in one fetch.
...
fn main() {
    ...
    // No leading "# generated by …" comment — NiFi's CSVReader doesn't
    // strip comment lines and would interpret the line as a data row.
    // The RNG_SEED constant above is the regeneration contract.

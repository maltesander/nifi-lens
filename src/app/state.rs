...
fn handle_key(state: &mut AppState, key: KeyEvent, config: &Config) -> UpdateResult {
    ...
    // Health view-local keys. Active only when no modal is open and we
    // are on the Health tab.
    if state.current_tab == ViewId::Health
        && state.modal.is_none()
        && let Some(r) = handle_health_key(state, key)
    {
        return r;
    }
...
// ---------------------------------------------------------------------------
// Health view-local key handler
// ---------------------------------------------------------------------------

fn handle_health_key(state: &mut AppState, key: KeyEvent) -> Option<UpdateResult> {
    ...
    match key.code {
        ...
        KeyCode::Down | KeyCode::Char('j') => match state.health.selected_category {
            HealthCategory::Queues => {
                let len = state.health.queues.rows.len();
                if len > 0 {
                    state.health.queues.selected = (state.health.queues.selected + 1) % len;
                }
            }
            HealthCategory::Nodes => {
                let len = state.health.nodes.nodes.len();
                if len > 0 {
                    state.health.nodes.selected = (state.health.nodes.selected + 1) % len;
                }
            }
            HealthCategory::Processors => {
                let len = state.health.processors.rows.len();
                if len > 0 {
                    state.health.processors.selected = (state.health.processors.selected + 1) % len;
                }
            }
        ...
        KeyCode::Up | KeyCode::Char('k') => match state.health.selected_category {
            HealthCategory::Queues => {
                let len = state.health.queues.rows.len();
                if len > 0 {
                    state.health.queues.selected = if state.health.queues.selected == 0 {
                        len - 1
                    } else {
                        state.health.queues.selected - 1
                    };
                }
            }
            HealthCategory::Nodes => {
                let len = state.health.nodes.nodes.len();
                if len > 0 {
                    state.health.nodes.selected = if state.health.nodes.selected == 0 {
                        len - 1
                    } else {
                        state.health.nodes.selected - 1
                    };
                }
            }
            HealthCategory::Processors => {
                let len = state.health.processors.rows.len();
                if len > 0 {
                    state.health.processors.selected = if state.health.processors.selected == 0 {
                        len - 1
                    } else {
                        state.health.processors.selected - 1
                    };
                }
            }
            HealthCategory::Repositories => {}
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Health key handler tests
    // -----------------------------------------------------------------------

    fn seeded_health_state() -> (AppState, Config) {
        use crate::client::health::{
            ConnectionStatusRow, FullPgStatusSnapshot, ProcessorStatusRow,
        };
        use crate::event::{HealthPayload, ViewPayload};
        use std::time::Instant;

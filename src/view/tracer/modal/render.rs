//! Private rendering helpers for the Tracer content viewer modal.
//!
//! Split from `modal/mod.rs` (formerly `modal.rs`) for navigability.
//! Visibility is `pub(super)` for items called from `mod.rs`; helpers
//! used only inside this module stay private. The only consumer is the
//! sibling `mod.rs` `render` entry point and its public chip-text
//! builders.
...
pub(super) fn render_header(frame: &mut Frame, area: Rect, modal: &ContentModalState) {
    let h = &modal.header;
    let at = format!(
        "at    {}   pg {}",
...
pub(super) fn render_tab_strip(frame: &mut Frame, area: Rect, modal: &ContentModalState) {
...
pub(super) fn render_body(frame: &mut Frame, area: Rect, modal: &ContentModalState) {
...
pub(super) struct RenderRow {
    pub(super) abs_idx: usize,
    pub(super) text: String,
    pub(super) base_style: ratatui::style::Style,
    pub(super) gutter: Span<'static>,
}
...
pub(super) fn render_stream_status(
    frame: &mut Frame,
    area: Rect,
    modal: &ContentModalState,
    ...
    let text = match buf {
        ...
        Some(b) if matches!(b.decoded, ContentRender::Tabular { .. }) => {
            super::footer_chip_text(&b.decoded, cfg, b.loaded.len())
        }
        Some(b) if b.ceiling_hit => super::ceiling_hit_chip(b, cfg),
...
pub(super) fn legacy_ceiling_hit_chip(
...
pub(super) fn render_search_strip(frame: &mut Frame, area: Rect, modal: &ContentModalState) {
